...
public class DispatcherExecutor {
    ...
    // cpu 密集型任务的线程池
    private static ThreadPoolExecutor cpuThreadPoolExecute;

    // io 密集型任务的线程池
    private static ExecutorService ioThreadPoolExecute;
    ...
    // 获取cup线程池
    public static ThreadPoolExecutor getCpuExecute() {
        return cpuThreadPoolExecute;
    }

    public static ExecutorService getIOExecute() {
        return ioThreadPoolExecute;
    }
    ...
    static {
        cpuThreadPoolExecute = new ThreadPoolExecutor(CORE_POOL_SIZE, MAXIMUM_POOL_SIZE, KEEP_ALIVE_SECONDS,
                TimeUnit.SECONDS, poolWorkQueue, threadFactory, handler);
        cpuThreadPoolExecute.allowCoreThreadTimeOut(true);
        ioThreadPoolExecute = Executors.newCachedThreadPool(threadFactory);
    }
}
